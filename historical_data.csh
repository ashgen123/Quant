...
date_end=$2
arg=$#
dir="/home/sachan/nse_Data/historical_fut_opt/"
...
end_file=fut_opt_${date_start}_${date_end}.csv
wget -O  $dir${date}.zip -U firefox http://nseindia.com/content/historical/DERIVATIVES/${year}/${month}/fo${date_format}bhav.csv.zip >> /tmp/log.txt 2>&1
	unzip $dir${date}.zip -d${dir}
        ...
        echo $count
        if [ $count -eq 1 ];then
                head -n1 ${dir}fo${day}${month}${year}bhav.csv > ${dir}${end_file}
        fi
        echo ${dir}fo${day}${month}${year}bhav.csv
        cat ${dir}fo${day}${month}${year}bhav.csv|tail -n+2 >>${dir}${end_file}
